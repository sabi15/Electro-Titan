...
async def team_register(interaction: discord.Interaction, division: str, team_name: str, team_code: str):
   ...
   if not div or div['status'] != 'active':
       await interaction.followup.send(embed=discord.Embed(
           description=f"{E_WRONG} Division not found or inactive.",
           color=0xe74c3c
       ))
       return

   accept_apps = await get_division_setting(div['id'], 'accept_applications')
   if not accept_apps or accept_apps.lower() != 'true':
       await interaction.followup.send(embed=discord.Embed(
           description=f"{E_WRONG} Applications are currently closed for this division.",
           color=0xe74c3c
       ))
       return
   ...
   if existing_code:
       await interaction.followup.send(embed=discord.Embed(
           description=f"{E_WRONG} Team code **{team_code}** is already taken.",
           color=0xe74c3c
       ))
       return
   ...
   await interaction.followup.send(embed=discord.Embed(
       description=f"{E_CORRECT} Application channel created: {channel.mention}",
       color=0x2ecc71
   ))
