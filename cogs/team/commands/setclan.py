...
async def team_setclan(interaction: discord.Interaction, clan_tag: str, clan_type: str):
    ...
    if clan_type == "secondary":
        ...
        if not allowed or allowed.lower() != 'true':
            await interaction.followup.send(embed=discord.Embed(
                description="❌ Secondary clans are not allowed in this division.",
                color=0xe74c3c
            ))
            return
    ...
    try:
        ...
        await interaction.followup.send(embed=discord.Embed(
            description="❌ Clan not found. Check the tag and try again.",
            color=0xe74c3c
        ))
        return
    ...
    if other:
        await interaction.followup.send(embed=discord.Embed(
            description=f"❌ Clan `{tag}` is already registered by team **{other['team_code']}**.",
            color=0xe74c3c
        ))
        return
    ...
    await interaction.followup.send(embed=discord.Embed(
        description=f"✅ **[{clan.name}]({clan_link})** — Lv.**{clan.level}** set as **{clan_type.capitalize()} Clan** for **{app['team_name']}**.",
        color=0x2ecc71
    ))
