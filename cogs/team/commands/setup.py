...
class TeamSetupModal(discord.ui.Modal):
    def __init__(self, team, div):
        ...
        self.team_code = discord.ui.TextInput(
            label="Team Code",
        ...
        self.clans = discord.ui.TextInput(
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        new_name = self.team_name.value.strip()
        new_code = self.team_code.value.strip().upper()
        new_language = self.language.value.strip()
        new_timezone = self.timezone.value.strip()
        clans_raw = self.clans.value.strip()
        ...
        # Check new code not taken by another team
        ...
        await pool.execute(
            ...
            WHERE id=$7
            """,
            new_name, new_code, new_language or None, new_timezone or None,
            new_main, new_secondary, team['id']
        )

        # Sync applications table if name/code changed
        await pool.execute(
...
async def team_setup(interaction: discord.Interaction, division: str, code: str):
    ...
    await interaction.followup.send(embed=discord.Embed(
        description=f"Opening setup for **{team['team_name']}**...",
        color=0x2b2d31
    ))
    await interaction.channel.send(view=discord.ui.View())
    modal = TeamSetupModal(team, div)
    await interaction.followup.send(view=_ModalLaunchView(modal))
