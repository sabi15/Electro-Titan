...
async def team_settimezone(interaction: discord.Interaction, timezone: str):
    ...
    await pool.execute(
        "UPDATE teams SET timezone=$1 WHERE guild_id=$2 AND division_code=$3 AND team_code=$4",
        timezone.strip(), gid, app['division_code'], app['team_code']
    )
    await interaction.followup.send(embed=discord.Embed(
        description=f"✅ Team timezone set to **{timezone.strip()}**.",
        color=0x2ecc71
    ))
